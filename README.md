...
| `DOTABUFF_BROWSER_ENABLED` | `0` | Включает Dotabuff-фолбэк через headless Chromium (один переиспользуемый worker-процесс) |
| `DOTABUFF_BROWSER_TOTAL_TIMEOUT_MS` | `90000` | Жёсткий дедлайн одного browser-запроса. По истечении вся process group воркера убивается SIGKILL и воркер пересоздаётся. Должен быть больше внутренних таймаутов страницы |
| `DOTABUFF_BROWSER_TIMEOUT_MS` | `20000` | Таймаут одной операции страницы (goto и т.п.) внутри воркера |
| `DOTABUFF_BROWSER_SETTLE_TIMEOUT_MS` | `15000` | Ожидание «устаканивания» страницы (Cloudflare и т.п.) |
| `DOTABUFF_BROWSER_IDLE_SHUTDOWN_SEC` | `900` | Через сколько секунд простоя убить резидентный Chromium (освобождает ~300-400MB). `0` = держать вечно |
| `DOTABUFF_BROWSER_ARTIFACTS` | `0` | Писать debug-артефакты (screenshot/html/json) на каждый fetch. Только для отладки |
| `DOTABUFF_BROWSER_OUTPUT_KEEP` | `50` | Сколько последних артефактов переживают рестарт воркера (bounded cleanup) |
