...
def _help_text() -> str:
    return (
        ...
        "\n"
        "Как часто: проверяет каждые 30 сек на таргет\n"
        "\n"
        "Команды:\n"
        "/setkey API_KEY - добавить Steam API ключ\n"
        "/add STEAMID64 ИМЯ - добавить таргет\n"
        "/remove STEAMID64 - удалить таргет\n"
        "/list - список таргетов и их статус\n"
        "/pause STEAMID64 - пауза мониторинга\n"
        "/resume STEAMID64 - возобновить\n"
        "/check STEAMID64 - мгновенная проверка профиля\n"
        "\n"
        "API ключ (бесплатно, домен — localhost):\n"
        "https://steamcommunity.com/dev/apikey\n"
        "\n"
        "SteamID64 (вставь ссылку на профиль):\n"
        "https://steamid.io\n"
        "\n"
        "Ограничение: профиль таргета должен быть Public.\n"
        "Private/Friends Only — API не отдаёт данные."
    )
